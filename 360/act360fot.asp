...
'response.write session("empn")
    vfe2 = vfecha
    'Enca
    sqlbi = "INSERT INTO ichequeos (fecha,servicio,piloto,unidad,observaciones,fechaOperacion, usuario) "
    sqlbi = sqlbi+"values('"& vfe2 & "'," & vtipos  & "," & vpiloto  & ",'" & vnunidad & "','"&vobs1 &"',now(), '"&session("Username")&"')"
	...
	while contador<=ubound(dato)
		...
		contador=contador+1
	wend
    SQL = "INSERT INTO ichequeosd (ncorre,idpregunta,valor,descripcion)values"&SQL
